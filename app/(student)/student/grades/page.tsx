    ...
    <div className="flex flex-col gap-6">
      <nav className="flex items-center justify-between text-xs text-slate-500 no-print">
        <div className="flex items-center gap-1.5 font-medium">
          ...
          <span>/</span>
          <span className="text-slate-900 dark:text-slate-100 font-semibold">Official Report Card</span>
        </div>
        <Button
          onClick={() => window.print()}
          variant="outline"
          size="sm"
          className="gap-1.5 border-slate-200 text-xs font-semibold hover:border-sky-300 hover:text-sky-700 dark:border-slate-800"
        >
          <Printer className="size-3.5" /> Print Report Card
        </Button>
      </nav>
      ...
      <div className="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-5 dark:border-slate-800">
          <div className="flex items-center gap-3.5">
            <div className="flex size-12 shrink-0 items-center justify-center rounded-2xl bg-sky-600 text-white shadow-sm shadow-sky-200">
          ...
          <div className="flex items-center gap-3">
            <div className="text-right">
              ...
              <p className="text-2xl font-black text-sky-700 dark:text-sky-400">Grade A*</p>
            </div>
            <div className="h-9 w-px bg-slate-200 dark:bg-slate-800" />
            <div className="text-right">
              <p className="text-xs text-slate-400">Percentage</p>
              <p className="text-2xl font-black text-slate-900 dark:text-slate-100">{percentage}%</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 pt-4 sm:grid-cols-4 text-xs">
          <div>
            ...
            <p className="font-bold text-slate-800 dark:text-slate-200 mt-0.5">Class 8 - Section A</p>
          </div>
          <div>
            <span className="text-slate-400">Roll Number</span>
            <p className="font-bold text-slate-800 dark:text-slate-200 mt-0.5">CH-08</p>
          </div>
          <div>
            <span className="text-slate-400">Total Marks</span>
            <p className="font-bold text-slate-800 dark:text-slate-200 mt-0.5">{totalObtained} / {totalMax}</p>
          </div>
          <div>
      ...
      <section className="rounded-2xl border border-slate-200/80 bg-white shadow-sm overflow-hidden dark:border-slate-800 dark:bg-slate-900">
        <div className="border-b border-slate-100 p-5 dark:border-slate-800">
          <h2 className="text-base font-bold text-slate-900 dark:text-slate-100">Subject Assessment Breakdown</h2>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {subjects.map((s) => (
                <tr key={s.code} className="hover:bg-slate-50/80 transition-colors dark:hover:bg-slate-800/50">
                  <td className="px-5 py-4 font-mono font-bold text-slate-500 dark:text-slate-400">{s.code}</td>
                  <td className="px-5 py-4 font-bold text-slate-900 dark:text-slate-100">{s.name}</td>
                  <td className="px-5 py-4 text-center text-slate-500">{s.teacher}</td>
                  <td className="px-5 py-4 text-center font-semibold text-slate-700 dark:text-slate-300">{s.mid}</td>
                  <td className="px-5 py-4 text-center font-semibold text-slate-700 dark:text-slate-300">{s.final}</td>
                  <td className="px-5 py-4 text-center font-extrabold text-slate-900 dark:text-slate-100">{s.total}</td>
                  <td className="px-5 py-4 text-center">
                    <span className="inline-block rounded-md bg-sky-50 px-2 py-0.5 font-black text-sky-700 dark:bg-sky-950 dark:text-sky-300">
                      {s.grade}
                    </span>
                  </td>
            ...
            <tfoot className="border-t-2 border-slate-200 bg-slate-50/80 font-bold dark:border-slate-800 dark:bg-slate-800/60">
              <tr>
                <td colSpan={5} className="px-5 py-3.5 text-slate-700 dark:text-slate-300">Aggregate Total</td>
                <td className="px-5 py-3.5 text-center text-sm font-extrabold text-sky-700 dark:text-sky-400">
