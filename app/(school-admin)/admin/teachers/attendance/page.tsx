    ...
    <div className="space-y-6">
      <nav className="flex items-center justify-between text-xs text-slate-500 no-print">
        <div className="flex items-center gap-1.5 font-medium">
          ...
          <span>/</span>
          <Link href="/admin/teachers" className="hover:text-emerald-600 transition text-slate-600 dark:text-slate-400">
            Faculty
          </Link>
          <span>/</span>
          <span className="text-slate-900 dark:text-slate-100 font-semibold">Faculty Attendance</span>
        </div>
      ...
      <header className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end no-print">
        <div>
          <span className="text-xs font-bold uppercase tracking-wider text-emerald-700 dark:text-emerald-400">Staff Haziri Register</span>
          <h1 className="mt-1 text-3xl font-extrabold tracking-tight text-slate-900 dark:text-slate-100">Faculty Daily Attendance</h1>
          <p className="text-slate-500 dark:text-slate-400">Record daily staff check-in timestamps and sync with monthly payroll deductions.</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={() => window.print()} className="rounded-xl border-slate-200">
            <Printer className="mr-1.5 size-4" /> Print Haziri Sheet
          </Button>
          <Button onClick={markAllPresent} className="bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl shadow-xs">
            <UserCheck className="mr-1.5 size-4" /> Mark All Present
          </Button>
        </div>
      </header>
      ...
      <section className="grid gap-4 grid-cols-2 sm:grid-cols-4">
        ...
        <article className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          ...
          <p className="mt-2 text-2xl font-black text-sky-600">{leaveCount}</p>
      ...
      <section className="rounded-2xl border border-slate-200/80 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 overflow-hidden">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 p-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/50">
          <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
            <Input
              type="date"
              value={selectedDate}
              onChange={(e) => setSelectedDate(e.target.value)}
              className="h-9 w-auto rounded-xl border-slate-200 text-xs font-medium"
            />
            <select
              value={department}
              onChange={(e) => setDepartment(e.target.value)}
              className="h-9 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-3 text-xs font-medium"
            >
              <option>All</option>
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {filtered.map((teacher) => (
                <tr key={teacher.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition">
                  <td className="px-5 py-3.5 font-mono text-slate-500 font-semibold">{teacher.code}</td>
                  <td className="px-5 py-3.5 font-bold text-slate-900 dark:text-slate-100">{teacher.name}</td>
                  <td className="px-5 py-3.5 text-slate-600 dark:text-slate-400">{teacher.department}</td>
                  <td className="px-5 py-3.5 font-mono text-slate-700 dark:text-slate-300">{teacher.timeIn}</td>
                  <td className="px-5 py-3.5">
                    <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[10px] font-bold ${
                      ...
                      teacher.status === 'Leave' ? 'bg-sky-50 text-sky-700 ring-1 ring-sky-500/20' :
                  ...
                  <td className="px-5 py-3.5 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button
                        ...
                        className={`size-7 rounded-lg text-xs font-bold transition ${teacher.status === 'Present' ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-emerald-50'}`}
                        title="Mark Present"
                      >
                        P
                      </button>
                      <button
                        ...
                        className={`size-7 rounded-lg text-xs font-bold transition ${teacher.status === 'Late' ? 'bg-amber-500 text-white' : 'bg-slate-100 text-slate-600 hover:bg-amber-50'}`}
                        title="Mark Late"
                      >
                        L
                      </button>
                      <button
                        ...
                        className={`size-7 rounded-lg text-xs font-bold transition ${teacher.status === 'Leave' ? 'bg-sky-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-sky-50'}`}
                        title="Mark Leave"
                      >
                        LV
                      </button>
                      <button
                        ...
                        className={`size-7 rounded-lg text-xs font-bold transition ${teacher.status === 'Absent' ? 'bg-rose-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-rose-50'}`}
                        title="Mark Absent"
                      >
                        A
                      </button>
