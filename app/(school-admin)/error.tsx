...
export default function SchoolAdminError({
  ...
  useEffect(() => {
    console.error('School Admin Portal Error:', error)
  }, [error])
    ...
    <div className="p-6 md:p-10 flex items-center justify-center min-h-[60vh]">
      <div className="w-full max-w-lg rounded-2xl border border-rose-200 bg-white p-6 sm:p-8 shadow-lg text-center">
        ...
        <h2 className="text-lg font-bold text-slate-900">
          Campus Operation Error
        </h2>
        <p className="mt-1.5 text-xs text-slate-600 leading-relaxed">
          {error.message || 'Unable to retrieve campus database records. The system does not serve fallback mock data. Please refresh or retry.'}
        </p>
        <div className="mt-6 flex items-center justify-center gap-3">
          ...
          >
            <RefreshCw className="size-3.5" /> Retry Operation
          </Button>
          <Link
            href="/admin"
