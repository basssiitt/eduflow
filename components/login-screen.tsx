...
export function LoginScreen() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    const useDemoFallback = () => {
      if (!demo) { setError('Invalid email or password'); setLoading(false); return }
    ...
    try {
      const { data: { user }, error: authError } = await supabaseClient.auth.signInWithPassword({ email: identifier.trim(), password: secret })
      if (authError || !user) {
        useDemoFallback()
        return
      }
      ...
      window.location.href = destinations[profile?.role ?? ''] ?? '/admin'
    } catch {
      useDemoFallback()
    }
  }
