...
export function PublicLanding() {
  ...
  return <main className="landing-page">
    <nav className="landing-nav">
      <Link className="landing-logo" href="/"><span className="crest-emblem"><AcademicCrest className="size-5" /></span><strong>EduFlow <em>OS</em></strong></Link>
      <div className="landing-links">
        <a href="#features" className="hover:text-[#2c1d17] transition-colors">Why EduFlow</a>
        <a href="#pricing" className="hover:text-[#2c1d17] transition-colors">Pricing</a>
        <a href="#faq" className="hover:text-[#2c1d17] transition-colors">FAQ</a>
        <Link href="/login?next=/admin" className="hidden sm:inline-block text-xs font-semibold text-[#2c1d17] dark:text-[#c5a059] hover:text-[#c5a059] transition-colors">Campus Admin</Link>
        <Link href="/login?next=/teacher" className="hidden sm:inline-block text-xs font-semibold text-[#2c1d17] dark:text-[#c5a059] hover:text-[#c5a059] transition-colors">Teacher</Link>
        <Link href="/login?next=/parent" className="hidden sm:inline-block text-xs font-semibold text-[#2c1d17] dark:text-[#c5a059] hover:text-[#c5a059] transition-colors">Parent</Link>
      </div>
      <div className="flex items-center gap-2">
        <Link className="text-xs font-semibold px-3 py-1.5 rounded-lg text-slate-700 hover:text-[#2c1d17] dark:text-slate-200 transition-colors" href="/login?force=1">
          Sign In
        </Link>
    ...
    <footer className="landing-footer">
      <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4 w-full py-8 text-left border-b border-slate-200/60 dark:border-slate-800">
        ...
        <div className="flex flex-col gap-1.5 text-xs">
          ...
          <Link href="/login?next=/admin" className="text-slate-600 hover:text-[#2c1d17] transition-colors">Admin Overview</Link>
          <Link href="/login?next=/admin/students" className="text-slate-600 hover:text-[#2c1d17] transition-colors">Student Register</Link>
          <Link href="/login?next=/admin/attendance" className="text-slate-600 hover:text-[#2c1d17] transition-colors">Daily Attendance</Link>
          <Link href="/login?next=/admin/fees" className="text-slate-600 hover:text-[#2c1d17] transition-colors">Fee Challans</Link>
          <Link href="/login?next=/admin/finance" className="text-slate-600 hover:text-[#2c1d17] transition-colors">Finance Ledger</Link>
          <Link href="/login?next=/admin/billing" className="text-slate-600 hover:text-[#2c1d17] transition-colors">Billing &amp; Plan</Link>
          <Link href="/login?next=/admin/settings" className="text-slate-600 hover:text-[#2c1d17] transition-colors">Campus Settings</Link>
        </div>
        <div className="flex flex-col gap-1.5 text-xs">
          <span className="font-bold text-slate-900 dark:text-slate-100 uppercase tracking-wider mb-1">Classroom &amp; Home</span>
          <Link href="/login?next=/teacher" className="text-slate-600 hover:text-[#2c1d17] transition-colors">Teacher Haziri Console</Link>
          <Link href="/login?next=/teacher/diary" className="text-slate-600 hover:text-[#2c1d17] transition-colors">Audio Voice Diary</Link>
          <Link href="/login?next=/teacher/gradebook" className="text-slate-600 hover:text-[#2c1d17] transition-colors">Gradebook &amp; Marks</Link>
          <Link href="/login?next=/parent" className="text-slate-600 hover:text-[#2c1d17] transition-colors">Parent Learning Space</Link>
          <Link href="/login?next=/parent#ai" className="text-slate-600 hover:text-[#2c1d17] transition-colors">24/7 AI Companion</Link>
        </div>
        <div className="flex flex-col gap-1.5 text-xs">
          <span className="font-bold text-slate-900 dark:text-slate-100 uppercase tracking-wider mb-1">Platform</span>
          <Link href="/signup" className="text-[#2c1d17] dark:text-[#c5a059] font-bold hover:underline">Start 30-Day Free Trial</Link>
          <Link href="/login?force=1" className="text-slate-600 hover:text-[#2c1d17] transition-colors">Account Sign In</Link>
          <Link href="/login?next=/super-admin" className="text-slate-600 hover:text-[#2c1d17] transition-colors">Super Admin Control Plane</Link>
          <a href={`${whatsapp}?text=Hello%20EduFlow%2C%20I%20want%20to%20book%20a%20demo.`} className="text-slate-600 hover:text-[#2c1d17] transition-colors">WhatsApp Support (03127803616)</a>
        </div>
      </div>
...
function Feature({ href, icon, title, text }: { href: string; icon: React.ReactNode; title: string; text: string }) {
  return (
    <Link href={href} className="feature-card group block no-underline transition hover:scale-[1.02] hover:shadow-md hover:border-[#c5a059]/50">
      <span className="feature-icon">{icon}</span>
      <h3>{title}</h3>
      <p>{text}</p>
      <span className="inline-flex items-center text-xs font-semibold text-[#2c1d17] dark:text-[#c5a059] mt-2">
