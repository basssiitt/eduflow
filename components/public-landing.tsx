    ...
    <footer className="landing-footer">
      <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4 w-full py-8 text-left border-b border-slate-200/60 dark:border-slate-800">
        ...
        <div className="flex flex-col gap-1.5 text-xs">
          ...
          <Link href="/login?next=/admin" className="text-slate-600 hover:text-emerald-600">Admin Overview</Link>
          <Link href="/login?next=/admin/students" className="text-slate-600 hover:text-emerald-600">Student Register</Link>
          <Link href="/login?next=/admin/attendance" className="text-slate-600 hover:text-emerald-600">Daily Attendance</Link>
          <Link href="/login?next=/admin/fees" className="text-slate-600 hover:text-emerald-600">Fee Challans</Link>
          <Link href="/login?next=/admin/finance" className="text-slate-600 hover:text-emerald-600">Finance Ledger</Link>
          <Link href="/login?next=/admin/billing" className="text-slate-600 hover:text-emerald-600">Billing &amp; Plan</Link>
          <Link href="/login?next=/admin/settings" className="text-slate-600 hover:text-emerald-600">Campus Settings</Link>
        </div>
        <div className="flex flex-col gap-1.5 text-xs">
          <span className="font-bold text-slate-900 dark:text-slate-100 uppercase tracking-wider mb-1">Classroom &amp; Home</span>
          <Link href="/login?next=/teacher" className="text-slate-600 hover:text-emerald-600">Teacher Haziri Console</Link>
          <Link href="/login?next=/teacher/diary" className="text-slate-600 hover:text-emerald-600">Audio Voice Diary</Link>
          <Link href="/login?next=/teacher/gradebook" className="text-slate-600 hover:text-emerald-600">Gradebook &amp; Marks</Link>
          <Link href="/login?next=/parent" className="text-slate-600 hover:text-emerald-600">Parent Learning Space</Link>
          <Link href="/login?next=/parent#ai" className="text-slate-600 hover:text-emerald-600">24/7 AI Companion</Link>
        </div>
        <div className="flex flex-col gap-1.5 text-xs">
          <span className="font-bold text-slate-900 dark:text-slate-100 uppercase tracking-wider mb-1">Platform</span>
          <Link href="/login?force=1" className="text-slate-600 hover:text-emerald-600">Account Sign In</Link>
          <Link href="/login?next=/super-admin" className="text-slate-600 hover:text-emerald-600">Super Admin Control Plane</Link>
          <a href={`${whatsapp}?text=Hello%20EduFlow%2C%20I%20want%20to%20book%20a%20demo.`} className="text-slate-600 hover:text-emerald-600">WhatsApp Support (03127803616)</a>
        </div>
      </div>
...
function Feature({ href, icon, title, text }: { href: string; icon: React.ReactNode; title: string; text: string }) {
  return (
    <Link href={href} className="feature-card group block no-underline transition hover:scale-[1.02]">
      <span className="feature-icon">{icon}</span>
      <h3>{title}</h3>
      <p>{text}</p>
      <span className="inline-flex items-center text-xs font-semibold text-emerald-600 dark:text-emerald-400 mt-2">
