    ...
    <div className="flex flex-col gap-6">
      <nav className="flex items-center justify-between text-xs text-slate-500 no-print">
        <div className="flex items-center gap-1.5 font-medium">
          <Link href="/teacher" className="hover:text-blue-600 transition flex items-center gap-1 text-slate-600">
            <ArrowLeft className="size-3.5" /> Classroom Haziri
          </Link>
          <span>/</span>
          <span className="text-slate-900 font-semibold">Gradebook &amp; Marks</span>
        </div>
        <div className="hidden sm:flex items-center gap-3">
          <Link href="/teacher/diary" className="hover:text-blue-600 transition font-medium text-slate-500">
            Audio Voice Diary →
          </Link>
        </div>
      </nav>
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-extrabold tracking-tight text-slate-900">Gradebook &amp; Marks Entry</h1>
          <p className="text-slate-500">Record assessment scores and term exam marks for student report cards.</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={() => window.print()} disabled={students.length === 0} className="border-slate-200 bg-white hover:bg-slate-50 text-slate-700">
            <Download className="mr-2 size-4" /> Print Marks Sheet
          </Button>
          <Button onClick={handleSave} className="bg-blue-600 hover:bg-blue-700 text-white font-semibold shadow-xs">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <label className="flex flex-col gap-1.5 text-xs font-semibold text-slate-700">
            ...
            <select
              ...
              className="h-10 rounded-xl border border-slate-200 bg-slate-50 px-3 text-sm font-medium text-slate-900 focus:border-blue-600"
            >
        ...
        <div className="mt-5 flex items-center justify-between">
          <div className="relative w-full sm:w-72">
            ...
            <Input
              ...
              className="pl-9 text-xs rounded-xl border-slate-200 bg-white text-slate-900 focus:border-blue-600"
            />
          </div>
                ...
                <tbody className="divide-y divide-slate-200">
                  {filtered.map((s) => (
                    <tr key={s.id} className="hover:bg-slate-50/70 transition-colors">
                      <td className="px-5 py-4 font-mono text-xs font-bold text-slate-900">{s.roll_no}</td>
                      <td className="px-5 py-4 font-semibold text-slate-900">{s.name}</td>
                      <td className="px-5 py-4 text-xs text-slate-600 font-medium">{s.class} - {s.section}</td>
                      <td className="px-5 py-4">
                        <Input
                          ...
                          placeholder="—"
                          className="h-8 text-xs font-mono text-center rounded-lg border-slate-200 bg-white text-slate-900 focus:border-blue-600"
                        />
                      </td>
                      <td className="px-5 py-4">
                        <Input
