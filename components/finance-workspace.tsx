    ...
    <main className="finance-page">
      <nav className="flex items-center justify-between text-xs text-slate-500 mb-6 no-print">
        <div className="flex items-center gap-1.5 font-medium">
          ...
          <span>/</span>
          <span className="text-slate-900 dark:text-slate-100 font-semibold">Finance &amp; Accounts Ledger</span>
        </div>
        <div className="hidden sm:flex items-center gap-3">
          ...
          <Link href="/admin/teachers/attendance" className="hover:text-emerald-600 transition font-medium text-slate-500">
            Faculty Attendance →
          </Link>
        </div>
      </nav>
      ...
      <header className="finance-page-header">
        ...
        <div className="finance-header-actions">
          <Button variant="outline" onClick={() => window.print()}><Printer className="mr-2 size-4" /> Print Financial Report</Button>
          <Button onClick={() => setModal("voucher")} className="bg-emerald-600 hover:bg-emerald-700 text-white shadow-xs"><Plus className="mr-2 size-4" /> Add Voucher</Button>
        </div>
      </header>
      ...
      {tab === "cash" && (
        <section className="finance-card cash-closing">
          ...
          <div className="p-6">
            <div className="max-w-md mx-auto p-5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-center">
              <Wallet className="size-10 text-emerald-600 mx-auto mb-2" />
              <h3 className="font-black text-lg text-slate-900 dark:text-slate-100">Daily Vault Balance</h3>
              <p className="text-xs text-slate-500 mt-1">Opening Cash: PKR 50,000 · Disbursed Today: PKR 14,200</p>
              <div className="my-4 py-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800">
                <span className="text-xs uppercase font-bold text-slate-400">Current Cash in Drawer</span>
                <strong className="block text-2xl font-black text-emerald-700 dark:text-emerald-400 mt-1">PKR 35,800</strong>
              </div>
              <Button onClick={() => notify("Petty cash reconciled and signed off for today.")} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl">
          ...
          <div className="w-full max-w-lg rounded-2xl bg-white dark:bg-slate-900 p-6 shadow-2xl border border-slate-200 dark:border-slate-800 print:border-none print:shadow-none print:p-0">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800 print:hidden">
              ...
              <div className="flex gap-2">
                <Button size="sm" onClick={() => window.print()} className="bg-emerald-600 text-white"><Printer className="size-3.5 mr-1" /> Print Slip</Button>
                <button onClick={() => setPaySlipStaff(null)} className="text-slate-400 hover:text-slate-600"><X className="size-5" /></button>
              </div>
            </div>
