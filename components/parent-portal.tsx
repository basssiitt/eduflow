...
export function ParentPortal() {
  ...
  return <main className="parent-page">
    ...
    <div className="parent-container">
      ...
      <section className="parent-report card-surface"><div className="section-heading"><div><span className="parent-eyebrow">Academic performance</span><h2>Term Examination &amp; Progress Report</h2></div><button className="parent-btn outline" onClick={() => setReportOpen(true)}><FileText /> View official report card</button></div><div className="marks-grid">{marks.map((item) => <div className="mark-card" key={item.subject}><div className={`mark-ring ${item.color}`}><strong>{item.mark}</strong><span>/100</span></div><b>{item.subject}</b><small>{item.mark >= 90 ? 'Excellent' : 'Very good'}</small></div>)}<div className="report-summary"><div><span>Term average</span><strong>88.7%</strong><small>Top 10% of class</small></div><div className="mini-chart" aria-label="Progress chart"><i /><i /><i /><i /><i /><i /></div></div></div></section>
