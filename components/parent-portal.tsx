...
function AiAssistant() {
  ...
  const send = async (text = input) => { if (!text.trim()) return; const now = '10:43 AM'; setMessages((m) => [...m, { role: 'user', text, time: now }]); setInput(''); setTyping(true); try { const response = await fetch('/api/ai/chat', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ prompt: text }) }); const result = await response.json(); setMessages((m) => [...m, { role: 'ai', text: result.text || aiReplies[text] || 'Main is sawal ko school records ke saath check karke aap ko update karta hoon.', time: now }]) } catch { setMessages((m) => [...m, { role: 'ai', text: aiReplies[text] || 'Main is sawal ko school records ke saath check karke aap ko update karta hoon.', time: now }]) } finally { setTyping(false) } }
...
export function ParentPortal() {
  ...
  return <main className="parent-page">
    ...
    <div className="parent-container">
      ...
      <section className="parent-status-grid"><div className="status-card attendance"><div className="status-icon"><CheckCircle2 /></div><div><span>Today&apos;s attendance</span><strong>{live?.attendance?.[0]?.status ?? 'Present'}</strong><small>{live ? `${Math.round((live.attendance.filter((row) => row.status === 'Present').length / Math.max(live.attendance.length, 1)) * 100)}% term attendance` : '94% term attendance'}</small></div><div className="status-pulse">Live</div></div><div className="status-card fee"><div className="status-icon"><Check /></div><div><span>August fee</span><strong>Paid <small>Rs. 2,500</small></strong><a href="#receipt" onClick={(e) => { e.preventDefault(); alert('Receipt download started for Sara Khan.') }}><Download /> Download receipt</a></div></div><div className="status-card class"><div className="status-icon"><CalendarDays /></div><div><span>Next class event</span><strong>Parent-teacher meeting</strong><small>Friday, 06 September · 3:30 PM</small></div></div></section>
